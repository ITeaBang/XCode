...
//  CoreData.181115
//
//  Created by 503-18 on 2018. 11. 15..
...
class ListVC: UITableViewController {
	...
	// Board Entity의 모든 데이터를 저장할 Property 생성
	...
	// 섹션의 개수를 설정하는 메소드를 삭제하거나 1을 리턴하도록 변경
    ...
    // 테이블 뷰에서 edit button을 누르고 나오는 버튼을 누를시 호출되는 메소드
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
		// 행 번호에 해당하는 데이터 찾기
		let object = self.list[indexPath.row]
		// 데이터 삭제
		if self.delete(object: object){
			// 메모리에서 행 번호에 해당하는 데이터 삭제
			// 한 곳에서만 사용하는 데이터베이스인 경우 가능,
			// 여러 곳에서 사용하는 데이터베이스의 경우 다시 불러 오는 것이 좋다.
	...
	override func tableView(_ tableView: UITableView, accessoryButtonTappedForRowWith indexPath: IndexPath){
		// 스토리보드에 만든 ViewController 인스턴스 생성 + 형변환
