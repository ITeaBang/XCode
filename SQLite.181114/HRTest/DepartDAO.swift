...
class DepartDAO{
    ...
    // 스위프트나 파이썬에서는 VO 클래스 대신에 튜플을 사용하는 것을 권장, 데이터 분석분야의 많은 메소드들이 튜플을 리턴
    ...
    lazy var fmdb : FMDatabase! = {
       // 파일 매니저 객체 가져오기
        ...
        // 파일의 존재 여부를 확인해서 파일이 없으면 번들에서 복사
	...
	func remove(departCd:Int) -> Bool{
		do{
			let sql = """
                ...
                where depart_cd = ?
        """
			try self.fmdb.executeUpdate(sql, withArgumentsIn: [departCd])
			return true
		}catch let error as NSError{
			print("삭제에러:\(error.localizedDescription)")
			return false
		}
	}
}
