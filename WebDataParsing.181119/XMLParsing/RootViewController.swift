...
class RootViewController: UITableViewController, XMLParserDelegate {
	...
	// Parsing에 필요한 변수, 태그 하나의 하나의 값을 저장할 변수
	...
	// elementValue의 값이 nil 이면 바로 저장하고 그렇지 않으면 이전 데이터에 추가해야 합니다.
	// 태그에 적는 내용은 하나의 패킷 이하로 만들어지기 때문에 한번에 전송되지만 여는 태그와 닫는 태그 사이에 작성하는 내용은 하나의 패킷 이상이 될 수 있어서이다.
